...
function PluginComponent() {
    ...
    const onFaceUpDown = (response) => {
        dialogPlugin.showAlertDialog("Event Information", response, openDialog);
    }

    const items = [
        {
            id: 1, label: 'Register', function: async () => {
                core.device.registerFaceUpDown({});
            }
        },
        {
            id: 1, label: 'Unregister', function: async () => {
                core.device.unregisterFaceUpDown({});
            }
        },
    ];

    useEffect(() => {
        core.addEventListener('onFaceUpDown', onFaceUpDown);

        return () => {
            core.removeEventListener('onFaceUpDown', onFaceUpDown);
        }
